import { withTrailingSlash, withoutTrailingSlash } from '@nuxt/ufo'
...
export default <%= options.blockLoading ? 'async ' : ''%>(context, inject) => {
  ...
  const trackRoute = ({ to, componentOption }) => {
    ...
    <% if(options.trailingSlash) { %>
      const fullPath = withTrailingSlash(to.fullPath)
    <% } else if(options.trailingSlash === false) { %>
      const fullPath = withoutTrailingSlash(to.fullPath)
    <% } else { %>
      const fullPath = to.fullPath
    <% } %>

    tracker.setCustomUrl(baseUrl + fullPath)
    ...
    <% if(debug || options.debug) { %>
    debug(`Tell matomo to track pageview ${fullPath}`, document.title)
