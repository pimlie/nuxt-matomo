...
export default <%= options.blockLoading ? 'async ' : ''%>(context, inject) => {
  ...
  if (window.Piwik) {
    ...
    let delayedCalls = []<% if(isTest) { %>// eslint-disable-line prefer-const<% } %>
    ...
    <% } else { %>
    tracker = {
      <%= options.apiMethodsList.reduce((acc, fn) => {
      return `${acc}${fn}: (...args) => proxyTrackerCall('${fn}', ...args),
      `
    }, '') %>
    }
    <% } %>
